...
export default function Layout({ children, festivalName = 'Ganesh Utsav' }) {
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col">
      {/* Tenant Inspection banner - now sits in normal flow at the very
          top of a non-scrolling root, so it's always visible without
          needing position:fixed or the margin-offset hacks that used to
          be sprinkled through the sidebar/header just to make room for it */}
      {user?.isInspecting && (
        <div className={`shrink-0 z-40 flex flex-col sm:flex-row items-center justify-between gap-2 px-4 py-2 text-sm font-medium ${
      ...
      {/* Everything below the banner fills exactly the remaining height
          and never scrolls as a unit - only the sidebar's nav list and
          the main content area scroll independently, on their own */}
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        {/* Desktop sidebar - locked to full available height, header and
            user-profile footer stay pinned; only the nav list in between
            scrolls if it ever grows taller than the screen */}
        <aside className="hidden md:flex md:flex-col w-64 shrink-0 h-full overflow-hidden bg-gradient-to-b from-maroon-700 to-maroon-800 text-white">
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
          {/* Top header - pinned, never scrolls; hamburger menu on mobile, welcome message on desktop */}
          <header className="shrink-0 z-30 bg-white/90 backdrop-blur border-b border-saffron-100 px-4 md:px-8 py-3 flex items-center justify-between">
